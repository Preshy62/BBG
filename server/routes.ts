...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/games/available", authenticate, async (req, res) => {
    try {
      ...
      // Get waiting games (joinable by anyone) 
      const waitingGames = await gameManager.getAvailableGames();
      
      // Get user's active games (games they're already part of)
      const userGames = await gameManager.getUserGames(req.session.userId);
      const activeUserGames = userGames.filter(game => 
        game.status === 'in_progress' || game.status === 'active'
      );
      
      // Combine and deduplicate
      const allAvailableGames = [...waitingGames];
      activeUserGames.forEach(userGame => {
        if (!allAvailableGames.find(g => g.id === userGame.id)) {
          allAvailableGames.push(userGame);
        }
      });
      
      console.log(`Available games fetched: ${allAvailableGames.length} games found`);
      allAvailableGames.forEach(game => {
        console.log(`Game ${game.id}: ${game.status}, ${game.maxPlayers} max players, stake: ${game.stake}`);
      });
      res.json(allAvailableGames);
    } catch (error) {
