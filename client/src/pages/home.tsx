    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="text-center p-4">
          ...
          <h3 className="font-bold">25% Win Chance</h3>
        ...
        <CardContent>
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-lg">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold">{formatCurrency(user.walletBalance)}</span>
            </div>
            <div className="flex justify-between items-center mt-2">
              <span>Selected Stake:</span>
              <span className="font-bold text-purple-600">{formatCurrency(selectedStake)}</span>
            </div>
            <div className="flex justify-between items-center mt-2">
              <span>Potential Win:</span>
              <span className="font-bold text-green-600">
                {formatCurrency(selectedStake * 2.5)}
              </span>
            </div>
          </div>
...
export default function Home() {
  ...
  const totalGamesPlayed = userGames?.length || 0;
  const completedGames = userGames?.filter(game => game.status === "completed") || [];
  const gamesWon = completedGames.filter(game => 
    game.winnerIds && Array.isArray(game.winnerIds) && game.winnerIds.includes(user?.id)
  ).length;
  const totalWinnings = transactions?.filter(t => 
    t.type === "winnings" && t.status === "completed"
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-grow container mx-auto px-2 sm:px-4 py-4 sm:py-8">
            ...
            <CardContent className="p-6">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <h3 className="text-2xl font-bold">{formatCurrency(totalWinnings)}</h3>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        {/* Bot Game Mode or Regular Games */}
        {isBotGameMode ? (
          <BotGameInterface 
            onCreateBotGame={createBotGameMutation.mutate}
          ...
          <div>
            ...
            <Card className="mb-4 sm:mb-6 bg-gradient-to-r from-blue-50 to-purple-50 border-2 border-dashed border-blue-200">
              <CardContent className="p-4 sm:p-6">
                <h3 className="text-lg sm:text-xl font-bold mb-3 sm:mb-4 text-center">Choose Your Game Mode</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <Button
                    onClick={() => setIsBotGameMode(true)}
                    className="h-16 sm:h-20 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white flex flex-col items-center justify-center gap-1 sm:gap-2"
                  >
                    <Gamepad2 className="h-5 w-5 sm:h-6 sm:w-6" />
                    <span className="font-bold text-sm sm:text-base">Play Against Bot</span>
                    <span className="text-xs opacity-90 hidden sm:block">25% Win Chance • Special Multipliers</span>
                  </Button>
                  
                  <Button
                    onClick={handleCreateGame}
                    variant="outline"
                    className="h-16 sm:h-20 border-2 border-blue-300 hover:bg-blue-50 flex flex-col items-center justify-center gap-1 sm:gap-2"
                  >
                    <Users className="h-5 w-5 sm:h-6 sm:w-6" />
                    <span className="font-bold text-sm sm:text-base">Multiplayer Game</span>
                    <span className="text-xs text-gray-600 hidden sm:block">2-10 Players • Voice Chat Available</span>
                  </Button>
                </div>
              </CardContent>
            </Card>
                    ...
                    <CardContent className="p-3 sm:p-4">
                      <div className="flex justify-between items-start mb-2 sm:mb-3">
                        <div>
                          ...
                          <p className="text-lg sm:text-2xl font-bold text-primary">{formatCurrency(game.stake)}</p>
                        </div>
                      ...
                      <div className="space-y-2 mb-4">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-bold text-green-600">{formatCurrency(game.stakePot)}</span>
                        </div>
