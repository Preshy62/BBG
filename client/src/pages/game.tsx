...
export default function GamePage() {
  ...
  const { data, isLoading, error } = useQuery<GameResponse>({
    ...
    enabled: !!gameId && !!user,
    refetchInterval: 1000, // Auto-refresh every second for real-time updates
    retry: (failureCount, error) => {
      // For bot games, keep trying to fetch data even if there are temporary errors
              ...
              <div className="space-y-3 sm:space-y-4 py-3 sm:py-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm sm:text-base font-medium">{formatCurrency(game.stake, game.currency)}</p>
                  </div>
                </div>
                    ...
                    <div className="bg-green-800 border-4 border-yellow-600 rounded-xl p-6 shadow-2xl relative overflow-visible" id="game-board">
                      ...
                      {game.status === "completed" && game.maxPlayers === 2 && (
                        <div className="text-center mt-4">
                          <button
                            onClick={() => {
                              // Create a new bot game with the same stake
                      ...
                      {/* Voice Chat and Text Chat Section */}
                      {game.stake >= 20000 && (
                        <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                          <h3 className="text-lg font-bold text-blue-800 mb-4 flex items-center">
                            🎤 Voice Chat {game.stake >= 50000 && <span className="ml-2 px-2 py-1 bg-gold-500 text-white text-xs rounded">PREMIUM</span>}
                          </h3>
                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            {/* Voice Chat Component */}
                            <div className="bg-white rounded-lg p-4 border">
                              <div className="text-center">
                                <p className="text-sm text-gray-600 mb-3">Voice chat is enabled for this game</p>
                            ...
                            {/* Text Chat */}
                            <div className="bg-white rounded-lg p-4 border">
                              <div className="h-32 bg-gray-50 rounded border mb-3 p-2 overflow-y-auto">
                                {messages.map((msg) => {
                                  const messageUser = players.find(p => p.userId === msg.userId)?.user;
                                  const username = messageUser?.username || 'Unknown';
                                  return (
                                    <div key={msg.id} className="text-sm mb-1">
                                      <span className="font-semibold text-blue-600">{username}:</span> {msg.content}
                                    </div>
                                  );
                                })}
                                {messages.length === 0 && (
                              ...
                              <div className="flex gap-2">
                                ...
                                <button 
                                  ...
                                  onClick={() => {
                                    const input = document.getElementById('chat-input-voice') as HTMLInputElement;
